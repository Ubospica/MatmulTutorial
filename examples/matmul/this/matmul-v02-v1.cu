...
__device__ void loadSmemB(half *smem, half *B, int N, int K, int ko)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        int col = tid % 4 * 8;
        // layout: [row_out, col_out, row_in, col_in] = [8, 2, 16, 16]
    // unsigned int addr = cast_smem_ptr_to_int(buf_dyn_shmem + ((((((ax0_ax1_fused_0_1 * 3072) + (((int)threadIdx.z) * 1536)) + (((((int)threadIdx.x) & 3) >> 1) * 768)) + (((int)threadIdx.y) * 384)) + ((((int)threadIdx.x) >> 2) * 48)) + ((((int)threadIdx.x) & 1) * 16)));
        void *ptr = (void *)(smem + row / 16 * (2 * 16 * 16) + col / 16 * (16 * 16) + row % 16 * 16 + col % 16);
        uint32_t smem_ptr;
...
__global__ void matmul(half *A, half *B, half *C, int M, int N, int K, float alpha, float beta)
{
    ...
    // unsigned int addr = cast_smem_ptr_to_int(buf_dyn_shmem + (((((((ax0_ax1_fused_0_3 * 3072) + (((int)threadIdx.z) * 1536)) + (((((int)threadIdx.x) & 3) >> 1) * 768)) + (((int)threadIdx.y) * 384)) + ((((int)threadIdx.x) >> 2) * 48)) + ((((int)threadIdx.x) & 1) * 16)) + 12288));
    // unsigned int addr = cast_smem_ptr_to_int(buf_dyn_shmem + (((((((ax0_ax1_fused_0_2 * 3072) + (((int)threadIdx.z) * 1536)) + (((((int)threadIdx.x) & 3) >> 1) * 768)) + (((int)threadIdx.y) * 384)) + ((((int)threadIdx.x) >> 2) * 48)) + ((((int)threadIdx.x) & 1) * 16)) + 61440));
